...
const canDrag = (cards, cardStacks, stackName, card) => {
...
const canDropOnSpace = (_, cardStacks, stackName, card) => {
...
const canDropOnSuit = (cardMap, cardStacks, stackName, card) => {
  const [fromStack, fromIndex] = findStack(cardStacks, card.id);
  const movingSequence = fromIndex < fromStack.cards.length - 1;
...
const canDropOnSpread = (cardMap, cardStacks, stackName, card) => {
  const [fromStack, fromIndex] = findStack(cardStacks, card.id);
  const movingSequence = fromIndex < fromStack.cards.length - 1;
